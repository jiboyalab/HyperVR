...
def ET_AADPPSSM(train_name, train_label, test_name,test_label, PSSM_Com_Feature):
    second_train_label_feature = []
    for second_i in range(train_label.shape[0]):
        pair = []
        if train_label[second_i] == "1":
            pair.append(1)
        else:
            pair.append(0)
        for second_i3 in range(len(PSSM_Com_Feature)):
            if train_name[second_i] == PSSM_Com_Feature[second_i3][0]:
                pair.extend(PSSM_Com_Feature[second_i3][1:])
                break
    ...
    for second_i in range(test_label.shape[0]):
        ...
        if test_label[second_i] == "1":
            ...
            pair.append(0)
        for second_i3 in range(len(PSSM_Com_Feature)):
            if test_name[second_i] == PSSM_Com_Feature[second_i3][0]:
                pair.extend(PSSM_Com_Feature[second_i3][1:])
                break
